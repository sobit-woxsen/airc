...
import { Mail, AlertCircle, CheckCircle, Loader2, Home, ChevronRight } from "lucide-react"
...
export default function LoginClient({ callbackUrl, error }: LoginClientProps) {
    ...
    return (
        <div className="flex min-h-screen bg-white overflow-hidden font-sans">
            ...
            <div className="hidden lg:flex lg:w-1/2 relative bg-neutral-900 items-center justify-center">
                <Image src="/ai-research-lab-interior.jpg" alt="AI Research Lab" fill className="object-cover opacity-80" priority />
                    ...
                    <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: "easeOut" }} className="space-y-6">
                        <div className="flex items-center gap-3 mb-8">
                            <div className="p-2 bg-white/10 backdrop-blur-md rounded-xl border border-white/20">
                                <Image src="/airc-logo-3d-latest.png" alt="AIRC" width={48} height={48} />
                            </div>
                        ...
                        <div className="pt-8 flex gap-8 items-center text-gray-400">
                            <div className="flex flex-col"><span className="text-white font-bold text-2xl">500+</span><span className="text-sm">Researchers</span></div>
                            <div className="w-px h-10 bg-white/10" />
                            <div className="flex flex-col"><span className="text-white font-bold text-2xl">100+</span><span className="text-sm">Projects</span></div>
                            <div className="w-px h-10 bg-white/10" />
                            <div className="flex flex-col"><span className="text-white font-bold text-2xl">24/7</span><span className="text-sm">Innovation</span></div>
                        </div>
                    </motion.div>
                </div>
            </div>
            ...
            <div className="w-full lg:w-1/2 flex flex-col relative bg-white">
                <header className="p-8 flex justify-between items-center absolute top-0 left-0 right-0 z-20">
                    ...
                    <Button variant="ghost" size="sm" onClick={() => router.push("/")} className="text-gray-500 hover:text-gray-900 transition-colors gap-2 ml-auto group rounded-full px-4">
                ...
                <main className="flex-1 flex items-center justify-center p-8 mt-12 lg:mt-0">
                    <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.4 }} className="w-full max-w-[400px] space-y-10">
                                ...
                                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="mb-8">
                                    <Alert variant="destructive" className="rounded-2xl border-red-100 bg-red-50 text-red-600">
                                        ...
                                        <AlertDescription className="font-medium">
                                            {error === "OAuthAccountNotLinked" ? "Email already in use with different provider" : "Authentication failed. Please try again."}
                                        </AlertDescription>
                                    </Alert>
                                </motion.div>
                            )}
                                    ...
                                    <div className="p-10 rounded-lg bg-neutral-50 border border-neutral-100 text-center space-y-6">
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <p className="text-gray-500 leading-relaxed px-4">
                                                ...
                                                <span className="block text-gray-900 font-semibold mt-2 break-all">
                                                    {magicLinkForm.getValues("email")}
                                                </span>
                                            </p>
                                        </div>
                                    </div>
                                    <Button variant="outline" onClick={() => { setMagicLinkSent(false); magicLinkForm.reset(); }} className="w-full border-gray-200 hover:bg-gray-50 hover:border-gray-300 text-gray-600 font-semibold py-7 rounded-lg hover:text-black cursor-pointer transition-all">
                                                ...
                                                <FormItem className="space-y-3">
                                                    ...
                                                    <FormControl>
                                                        <div className="relative group">
                                                            ...
                                                            <Input {...field} type="email" placeholder="name@company.com" className="pl-14 py-6 bg-neutral-50 border-neutral-200 text-gray-900 text-lg placeholder:text-gray-400 focus:ring-4 focus:ring-[#52c2cb]/10 focus:border-[#52c2cb] focus:bg-white transition-all rounded-sm" disabled={isLoading} />
                                                        </div>
                                                    </FormControl>
                                                    <FormMessage className="text-red-500 font-medium ml-1" />
                                                </FormItem>
                                            )} />
                                            <Button type="submit" className="w-full bg-gray-900 hover:bg-black text-white  py-6 text-sm rounded-sm transition-all shadow-xl shadow-gray-200 group relative overflow-hidden active:scale-[0.98]" disabled={isLoading}>
                                                ...
                                                {isLoading ? <Loader2 className="h-6 w-6 animate-spin mx-auto" /> : <span className="flex items-center justify-center gap-3">Send Magic Link<ChevronRight className="h-6 w-6 group-hover:translate-x-1 transition-transform" /></span>}
                                            </Button>
                                            <div className="pt-2">
